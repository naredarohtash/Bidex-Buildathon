...
export const WebSocketProvider = ({
  ...
  const playNotificationSound = useCallback(() => {
    // Check global mute state from useTradingSettingsStore
    ...
    if (isMutedGlobal || !soundEnabledRef.current || !audioRef.current || isPlayingRef.current) {
      return;
    }
  ...
  useEffect(() => {
    ...
    const messageHandler = (msg: any) => {
      ...
      if (msg.stream === "notification" && msg.data) {
        ...
        /* Not for trade notices — the terminal has its own sound for those.

           This plays /sound/notification.mp3, the same file the support system
           uses, for every notification the server pushes. The backend raises one
           when a position settles, so a settled trade produced that generic ping
           on top of the terminal's own tone: identical to the support sound,
           which is exactly how it was recognised.

           Matched on content rather than on being in the terminal, so a support
           message still pings while a trader is on the chart. */
        const d: any = msg.data || {};
        const kind = String(d.type || d.category || "").toLowerCase();
        const text = `${d.title || ""} ${d.message || ""}`.toLowerCase();
        const isTradeNotice =
          /order|trade|binary|position/.test(kind) ||
          /\btrade\b|\bposition\b|\border\b|won|lost|expired|refund/.test(text);
        if (!isTradeNotice) {
