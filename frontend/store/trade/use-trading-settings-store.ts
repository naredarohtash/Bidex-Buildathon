...
export const useTradingSettingsStore = create<TradingSettingsState>()(
  persist(
    ...
    }),
    {
      ...
      /* Deep-merge, because zustand's default merge is shallow.

         `audio.sounds` is a map of sound name to boolean, and AudioFeedback.play
         bails on `!this.config.sounds[name]`. A browser holding an `audio`
         object written before a sound existed therefore has `undefined` for it
         and plays nothing — silently, and forever, because the persisted object
         replaces the defaults wholesale on every load rather than filling in
         around them. That is a settled trade making no sound at all while the
         defaults in the source say every sound is on.

         Merging per key keeps a preference the user actually set and gives
         anything they have never seen its default. */
      merge: (persisted: any, current) => {
        ...
        return {
          ...
          audio: {
            ...current.audio,
            ...(persisted.audio || {}),
            sounds: { ...current.audio.sounds, ...((persisted.audio || {}).sounds || {}) },
          },
          oneClick: { ...current.oneClick, ...(persisted.oneClick || {}) },
          martingale: { ...current.martingale, ...(persisted.martingale || {}) },
          notifications: { ...current.notifications, ...(persisted.notifications || {}) },
        };
      },
    }
  )
