  ...
  return (
    <button
      ...
      className={`w-full text-left p-3 rounded-lg transition-all ${
        isSelected
          ? isDark
            ? "bg-zinc-800 border-2 border-[#0052ff]"
            : "bg-gray-100 border-2 border-[#0052ff]"
          : isDark
        ...
        <ul className="space-y-1.5">
          {pattern.tradingTips.map((tip, i) => (
            <li key={i} className="flex items-start gap-2">
              <ChevronRight
                className={`w-3.5 h-3.5 mt-0.5 flex-shrink-0 ${
                  isDark ? "text-[#0052ff]" : "text-blue-600"
                }`}
              />
              <span
                className={`text-sm ${
...
export default function PatternLibrary({
  ...
  const content = (
    <Wrapper
      {...wrapperProps}
      /* A section is the screen: top edge to the navigation bar. It used to
         start at top-[53px], which left the chart's floating header showing
         above it — so a leaderboard was drawn under a live asset selector, and
         the terminal's own back button landed on top of the balance box. It
         stops at the bar rather than covering it, so the navigation stays
         reachable from inside the section. */
      style={isMobile ? { bottom: MOBILE_NAV_HEIGHT } : undefined}
      className={`absolute top-0 ${isMobile ? "" : "bottom-0"} left-0 right-0 z-50 flex pointer-events-none`}
    >
      {/* Backdrop */}
      <BackdropWrapper
        {...backdropProps}
      ...
      <PanelWrapper
        ...
        className={`relative ${
          isMobile 
            ? `ml-auto w-full h-full border-l ${isDark ? "bg-zinc-900" : "bg-white"}` 
            : `mr-auto w-[361px] h-full rounded-none border-y border-r ${isDark ? "bg-[#13151f]/80" : "bg-white/80"} backdrop-blur-md`
        } flex flex-col shadow-2xl ${borderClass} pointer-events-auto`}
      >
            {/* Header */}
            <div className={`flex items-center justify-between px-4 py-3.5 border-b ${borderClass}`}>
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h2 className={`text-sm font-semibold ${isDark ? "text-white" : "text-gray-900"} leading-none`}>
                    {t("pattern_library")}
                  </h2>
                  <p className={`text-[10px] ${isDark ? "text-zinc-500" : "text-gray-500"} leading-none mt-1`}>
            ...
            {/* Stats bar */}
            <div className={`px-4 py-2 border-b ${borderClass} ${isDark ? "bg-zinc-800/20" : "bg-gray-50"}`}>
            ...
            <div className={`px-4 py-2 border-b ${borderClass}`}>
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full pl-9 pr-4 py-1.5 text-xs rounded-lg ${
                    isDark
                  ...
                  } border focus:outline-none focus:ring-2 focus:ring-[#0052ff]/50`}
                />
              </div>
            </div>

            {/* Category Filter Row */}
            <div className={`flex items-center gap-1 px-4 py-2 border-b ${borderClass} overflow-x-auto scrollbar-none`}>
                ...
                <div className="flex-1 overflow-y-auto p-4">
                  {selectedPattern && (
                    <button
                      ...
                      className={`flex items-center gap-2 mb-4 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
