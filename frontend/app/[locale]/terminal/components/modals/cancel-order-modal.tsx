...
export function CancelOrderModal({
  ...
  const refundAmount = (order.amount - cancellationFee) * preferredCurrencyRate;
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="rounded-lg bg-zinc-800/50 p-4 space-y-3">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="text-white font-medium">
                {formatCurrency(order.amount * preferredCurrencyRate)} {preferredCurrency}
              </span>
            </div>
            <div className="flex justify-between text-sm">
          ...
          <div className="rounded-lg bg-zinc-800/50 p-4 space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="text-white">{formatCurrency(order.amount * preferredCurrencyRate)} {preferredCurrency}</span>
            </div>
            {cancellationFee > 0 && (
              <div className="flex justify-between text-sm">
                <span className="text-zinc-400">{tBinary("cancellation_fee") || "Cancellation Fee"}:</span>
                <span className="text-red-400">-{formatCurrency(cancellationFee * preferredCurrencyRate)} {preferredCurrency}</span>
              </div>
            )}
            <div className="border-t border-zinc-700 pt-2 mt-2">
              <div className="flex justify-between">
                <span className="text-zinc-300 font-medium">{t("refund_amount") || "Refund Amount"}:</span>
                <span className="text-green-400 font-bold">{formatCurrency(refundAmount)} {preferredCurrency}</span>
              </div>
            </div>
          </div>
