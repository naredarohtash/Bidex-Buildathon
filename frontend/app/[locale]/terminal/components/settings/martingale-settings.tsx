...
import { useMartingaleStore } from "@/store/trade/use-martingale-store";
    ...
    return (
      <>
        <div className={`space-y-4 ${className}`}>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <Label className={`text-[11px] font-bold uppercase tracking-wider ${mutedClass}`}>Stake Multiplier (On Loss)</Label>
              <span className="text-sm font-bold text-orange-500">{state.multiplier}x</span>
            </div>
            <Slider
              ...
              className="mt-1"
            />
            <div className={`flex justify-between text-[10px] mt-1.5 font-bold ${mutedClass}`}>
              <span>1.5x</span>
              <span>2x</span>
              <span>2.5x</span>
              <span>3x</span>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <Label className={`text-[11px] font-bold uppercase tracking-wider ${mutedClass}`}>Max Recovery Steps</Label>
              <span className={`text-sm font-bold ${textClass}`}>{state.maxConsecutiveIncreases} Steps</span>
            </div>
            <Slider
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-1.5">
                <Label className={`text-[11px] font-bold uppercase tracking-wider ${mutedClass}`}>Max Risk Safety Limit</Label>
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger>
              ...
              <span className="text-sm font-bold text-red-500">{state.stopLossLimit}%</span>
            </div>
            <Slider
              ...
              className="mt-1"
            />
            <div className={`text-[10px] mt-1.5 font-bold ${mutedClass}`}>
              Max allowed trade: {((balance * state.stopLossLimit) / 100).toLocaleString(undefined, { maximumFractionDigits: 0 })} {balance > 0 ? "USDT" : ""}
            </div>
          </div>

          {/* Expected Multiplication Progression Table */}
          <div className={`rounded-xl border ${borderClass} ${bgClass} p-3.5 space-y-2.5 font-sans`}>
            <div className="text-[10px] font-bold text-zinc-400 dark:text-zinc-300 uppercase tracking-wider">Multiplication Progression</div>
            <div className="flex items-center gap-1.5 overflow-x-auto pb-1.5 scrollbar-custom font-mono">
              {Array.from({ length: Math.min(5, state.maxConsecutiveIncreases + 1) }).map((_, i) => {
                const stepAmount = Math.round((state.baseAmount || currentAmount) * Math.pow(state.multiplier, i));
                return (
                  <div key={i} className="flex items-center shrink-0">
                    {i > 0 && <span className="text-zinc-650 mx-1 text-xs">→</span>}
                    <div className="flex flex-col items-center p-2 rounded-lg bg-zinc-950 border border-border/40 select-none">
                      <span className="text-[9px] text-zinc-500 font-bold uppercase tracking-wider font-sans">Loss {i}</span>
                      <span className="font-extrabold text-[11px] text-white mt-0.5">{stepAmount.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
                    </div>
                  </div>
                );
              })}
              {state.maxConsecutiveIncreases >= 5 && (
                <div className="flex items-center shrink-0">
                  <span className="text-zinc-650 mx-1 text-xs">→</span>
                  <div className="flex items-center justify-center p-2.5 rounded-lg bg-zinc-950/40 border border-dashed border-border/40 select-none">
                    <span className="text-[10px] text-zinc-500 font-bold">+{state.maxConsecutiveIncreases - 4} Steps</span>
                  </div>
                </div>
              )}
            </div>
            <p className="text-[10px] text-zinc-450 dark:text-zinc-500 leading-normal font-semibold">Stakes will multiply after losses and reset immediately to your base amount of {(state.baseAmount || currentAmount).toLocaleString()} once you win.</p>
          </div>

          {/* Reset on Win */}
          <div className={`flex items-center justify-between p-3.5 rounded-xl border ${borderClass} ${darkMode ? "bg-zinc-800/40" : "bg-gray-100"}`}>
            <div className="flex items-center gap-1.5">
              <Label htmlFor="reset-on-win-expanded" className={`text-sm cursor-pointer font-bold ${textClass}`}>
                Reset on Win
              </Label>
              <TooltipProvider>
                <Tooltip>
          ...
          <div className={`border-t ${borderClass} p-3 space-y-4`}>
            ...
            <div>
              ...
              <Slider
                ...
                className="mt-1"
              />
              <div className={`flex justify-between text-[10px] mt-1 ${mutedClass}`}>
                <span>1.5x</span>
                <span>2x</span>
                <span>2.5x</span>
                <span>3x</span>
...
// Hook for using martingale state - uses global stores for persistence and runtime sharing
export function useMartingale(initialAmount: number = 100) {
  ...
  // Global Zustand store for martingale runtime shared state
  const runtimeState = useMartingaleStore((s) => s.state);
  const setRuntimeState = useMartingaleStore((s) => s.setState);
  ...
  const state: MartingaleState = {
    ...
    stopLossLimit: globalMartingale.stopLossLimit || 50,
  ...
  const setState = useCallback((newState: MartingaleState | ((prev: MartingaleState) => MartingaleState)) => {
    ...
    updateMartingale({
      ...
      stopLossLimit: resolvedState.stopLossLimit,
    });
  ...
  }, [state, globalMartingale.enabled, setMartingaleEnabled, updateMartingale, setRuntimeState]);
