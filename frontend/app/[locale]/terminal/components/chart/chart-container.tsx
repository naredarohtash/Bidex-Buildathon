...
// Touch trigger: Selectors simplified to w-full flex-1 overflow-hidden layout
import { useEffect, useRef, useCallback, useMemo, memo } from "react";
import type { Symbol, TimeFrame, OrderSide } from "@/store/trade/use-binary-store";
import { isSameSymbol } from "@/store/trade/use-binary-store";
...
interface ChartContainerProps {
  ...
  isMobile?: boolean;
  /** Mobile only — shows/hides the engine's left drawing rail. See ChartSwitcher. */
...
export const ChartContainer = memo(function ChartContainer({
  ...
  const chartProps = useMemo(() => ({
    symbol,
    timeFrame,
    orders: orders.filter((order) => isSameSymbol(order.symbol, symbol)),
    expiryMinutes,
    showExpiry,
    timeframeDurations,
    positions,
    ...
    showDrawingTools, // Mobile only — the engine's left drawing rail
  ...
  }), [symbol, timeFrame, orders, expiryMinutes, showExpiry, timeframeDurations, positions, isMarketSwitching, currency, defaultOrderAmount, onPlaceOrder, isMobile, showDrawingTools, onOpenNotificationSettings, onCloseParentOverlays, closeInternalOverlays]);
  ...
  return (
    <div data-tutorial="chart-area" className={`w-full h-full ${isMobile ? "z-0" : ""}`} style={{ position: "relative", width: "100%", height: "100%", display: "flex", flexDirection: "column" }}>
