...
import { useRiskStore } from "@/store/trade/use-risk-store";
...
export function useRiskManagement(
  ...
  // Load state and setState from global Zustand store for live multi-instance synchronization
  const state = useRiskStore((s) => s.state);
  const setState = useRiskStore((s) => s.setState);
  ...
  /* Cooldown expiry.
     ...
     This used to be a bare `now >= endsAt` check with no timer behind it, and
     its dependencies are the cooldown's own start and end — so the only thing
     that could ever run it again was some *other* state change happening to
     re-render the hook. Nothing does that while a trader sits and waits out the
     pause. A fifteen-minute cooldown therefore stayed `isInCooldown: true` past
     its end, and `canTrade()` reads that flag directly rather than the clock, so
     trading stayed blocked indefinitely — until a reload, or an unrelated
     re-render, happened to let this line run.
     
     Scheduled now, so the pause ends when it says it ends. The extra 250ms is
     so the timer never fires a hair early and re-arms itself for one more tick. */
  useEffect(() => {
    if (!state.cooldown.isInCooldown || !state.cooldown.cooldownEndsAt) return;

    const endCooldown = () =>
      setState((prev) => ({
        ...prev,
        cooldown: { ...prev.cooldown, isInCooldown: false, cooldownEndsAt: undefined },
      }));

    const remaining = state.cooldown.cooldownEndsAt - getChartSynchronizedTime().getTime();
    if (remaining <= 0) {
      endCooldown();
      return;
    }

    const timer = setTimeout(endCooldown, remaining + 250);
  ...
  const updateDailyLimit = useCallback((settings: Partial<DailyLimitSettings>) => {
    setState((prev) => {
      const mergedLimit = { ...prev.dailyLimit, ...settings };
      const currentPL = mergedLimit.currentDailyPL;
      const maxLoss =
        mergedLimit.maxDailyLossType === "percentage"
          ? (balance * mergedLimit.maxDailyLoss) / 100
          : mergedLimit.maxDailyLoss;

      const isLimitReached = mergedLimit.enabled && -currentPL >= maxLoss;
  ...
  const recordLoss = useCallback(() => {
    ...
    setState((prev) => {
      ...
      if (shouldStartCooldown && !prev.cooldown.isInCooldown) {
        const endsAt = getChartSynchronizedTime().getTime() + prev.cooldown.cooldownMinutes * 60 * 1000;
  ...
  const isInCooldown = useCallback((): boolean => {
    ...
    return getChartSynchronizedTime().getTime() < state.cooldown.cooldownEndsAt;
  }, [state.cooldown]);

  const getCooldownRemaining = useCallback((): number => {
    if (!state.cooldown.cooldownEndsAt) return 0;
    return Math.max(0, state.cooldown.cooldownEndsAt - getChartSynchronizedTime().getTime());
