...
export const TradeCooldown = memo(function TradeCooldown({
  ...
  useEffect(() => {
    ...
    const updateTimer = () => {
      const remaining = Math.max(0, settings.cooldownEndsAt! - getChartSynchronizedTime().getTime());
  ...
  // Calculate progress (shrinking indicator as time remaining runs out)
  const progress = settings.isInCooldown && settings.cooldownEndsAt
    ? Math.max(0, Math.min(1, timeRemaining / (settings.cooldownMinutes * 60 * 1000)))
    : 0;
