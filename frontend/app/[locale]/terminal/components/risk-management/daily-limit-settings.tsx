...
import { CURRENCY_SYMBOLS } from "../header/header";
...
export const DailyLimitSettingsComponent = memo(function DailyLimitSettingsComponent({
  ...
  // Format currency andtimezone dynamically matching user specs
  ...
  const infoText = useMemo(() => {
    const base = t("limit_resets_daily_at_midnight_local");
    const tzSuffix = timezoneText ? `${timezoneText}` : "local time";
    // Replace "midnight (local time)" with "12:00 AM (TZ)"
    if (base.includes("midnight (local time)")) {
      return base.replace("midnight (local time)", `12:00 AM (${tzSuffix})`);
    }
    // Fallback if translations are different: replace "local time" or just append
    return base.replace("local time", tzSuffix);
  }, [t, timezoneText]);
          ...
          >
            ...
            <div className="flex items-center justify-between mt-2">
              <span className={`text-[10px] ${subtitleClass}`}>
                {tCommon("remaining")} {status.remaining.toFixed(2)} {currency}
              </span>
              {status.isReached && (
        ...
        >
          ...
          <span className={`text-[10px] ${subtitleClass}`}>
            ...
            >
              ...
              {settings.currentDailyPL.toFixed(2)} {currency}
            </span>
          </span>
        </div>
      </div>
