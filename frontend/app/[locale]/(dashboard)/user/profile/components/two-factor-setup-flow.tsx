...
interface TwoFactorSetupFlowProps {
  onCancel: () => void;
  onComplete: () => void;
  /* Skip the method picker when the caller already knows which one was asked
     for — the security page turns each method on from its own row, so being
     dropped back onto a list of three is a step that answers a question the
     user has already answered. */
  initialMethod?: "APP" | "SMS" | "EMAIL";
}
...
export function TwoFactorSetupFlow({
  ...
  const [method, setMethod] = useState<TwoFactorMethod>(
    initialMethod || (user?.twoFactor?.type as TwoFactorMethod) || null
  );
  ...
  /* Straight into the chosen method's setup. `method` is already that method
     from its initial state, and the guard keeps a re-render from asking the
     server for a second secret. */
  const autoStarted = useRef(false);
  useEffect(() => {
    if (!initialMethod || autoStarted.current) return;
