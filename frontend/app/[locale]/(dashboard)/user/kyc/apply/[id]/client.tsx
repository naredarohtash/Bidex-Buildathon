...
/* `levelId` and `onExit` are optional so this form can be mounted inside the
   account panel as well as served at /user/kyc/apply/[id]. Without them it
   reads the id from the route and navigates on completion, exactly as before;
   with them it stays where it is put and hands control back to its host. That
   is what lets KYC live in the panel instead of sending the trader to another
   page mid-flow. */
export function KycApplicationClient({
  ...
  const id = levelId ?? routeParams?.id ?? "";
  ...
  const fetchLevel = async () => {
    try {
      ...
      if (
        ...
        onExit ? onExit() : router.push("/user/kyc");
        return;
      }
  ...
  const handleFinalSubmit = async (formData: Record<string, any>) => {
    ...
    try {
      ...
      onExit ? onExit() : router.push(`/user/kyc/application/${data.application.id}`);
    } catch (error) {
  ...
  if (!level) {
    return (
      <div className="container max-w-4xl py-12 dark:bg-zinc-950">
        ...
        <Button className="mt-6" onClick={() => (onExit ? onExit() : router.push("/user/kyc"))}>
      ...
      <div className="flex flex-col gap-8">
        ...
        <div className="flex items-center gap-4">
          <Button
            ...
            onClick={() => (onExit ? onExit() : router.push("/user/kyc"))}
          >
