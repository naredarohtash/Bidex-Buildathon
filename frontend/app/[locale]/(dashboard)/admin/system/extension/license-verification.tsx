...
export function LicenseVerification() {
  ...
  const handleActivateLicense = async () => {
    if (!purchaseCode.trim() || !bidexUsername.trim()) {
      return;
    }

    setIsSubmitting(true);
    await activateLicense(purchaseCode, bidexUsername, notificationEmail.trim() || undefined);
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          <label htmlFor="bidex-username" className="text-sm font-medium">
            {tDashboardAdmin("bidex_username")}
          </label>
          <Input
            id="bidex-username"
            value={bidexUsername}
            onChange={(e) => setBIDEXUsername(e.target.value)}
            placeholder={t("enter_your_bidex_username")}
            disabled={isSubmitting}
          />
        </div>
        <div className="space-y-2">
        ...
        <Button
          ...
          disabled={isSubmitting || !purchaseCode.trim() || !bidexUsername.trim()}
        >
          {isSubmitting ? (
            <>
