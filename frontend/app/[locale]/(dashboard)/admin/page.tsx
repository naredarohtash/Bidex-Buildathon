...
function QuickActions() {
  ...
  const actions = [
    ...
    { icon: BarChart3, label: "Markets", path: "/admin/finance/binary/market", color: "text-cyan-600 dark:text-cyan-400" },
      ...
      if (batchResult?.products && Array.isArray(batchResult.products)) {
        ...
        const updateChecks: UpdateInfo[] = batchResult.products
          ...
          .map((product: any) => {
            ...
            return {
              productId,
              name: ext?.name || (isCore ? "bidex" : "unknown"),
              title: ext?.title || (isCore ? "BIDEX Core" : productId),
              currentVersion,
              latestVersion,
