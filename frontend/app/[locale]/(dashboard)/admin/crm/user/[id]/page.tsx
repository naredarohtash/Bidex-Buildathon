...
import { useColumns as useBinaryOrderColumns } from "@/app/[locale]/(dashboard)/admin/finance/binary-order-columns";
import { useColumns as useExchangeOrderColumns } from "@/lib/stubs/ext-columns-stub";
import { useColumns as useFuturesOrderColumns } from "@/lib/stubs/ext-columns-stub";
import { useColumns as useEcosystemOrderColumns } from "@/lib/stubs/ext-columns-stub";
import { useColumns as useInvestmentColumns } from "@/lib/stubs/ext-columns-stub";
// ICO was removed along with its pages, so there are no columns to import. The
// tab that used these is gated on the "ico" extension and no longer appears at
// all; this stub only keeps the surrounding table wiring type-correct rather
// than unpicking a 2,000-line page for a panel that cannot render.
const useIcoTransactionColumns = (): any[] => [];
import { useColumns as useP2pOfferColumns } from "@/lib/stubs/ext-columns-stub";
import { useColumns as useP2pTradeColumns } from "@/lib/stubs/ext-columns-stub";
/* Staking, ecommerce and forex are gone; their column hooks come from the stub
   that exists for exactly this. It returns an empty array, and the availability
   check below hides a tab whose columns are empty — so the CRM page loses those
   three tabs without a single conditional being written. Affiliate keeps its
   real import. */
import { useColumns as useStakingPositionColumns } from "@/lib/stubs/ext-columns-stub";
import { useColumns as useAffiliateReferralColumns } from "@/app/[locale]/(ext)/admin/affiliate/referral/columns";
import { useColumns as useEcommerceOrderColumns } from "@/lib/stubs/ext-columns-stub";
import { useColumns as useForexTransactionColumns } from "@/lib/stubs/ext-columns-stub";
