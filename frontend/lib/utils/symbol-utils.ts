...
/**
 * Normalize a Vortex symbol format (e.g. "AUD/CAD/OTC", "BTC/USDT/OTC", "AAPL/USD/OTC") 
 * to BideX canonical format (e.g. "AUD/CAD (OTC)", "BTC/USD (OTC)", "AAPL/USD (OTC)").
 */
export function vortexToBidexSymbol(symbol: string): string {
  ...
  // Count slashes in original symbol before cleaning
  const slashCount = (symbol.match(/\//g) || []).length;

  // Normalize separators and strip existing OTC suffix variations
  let clean = symbol.replace(/-/g, "/").replace(/_OTC$/i, "").replace(/\/?\s*\(?OTC\)?$/gi, "").trim();
  ...
  if (clean.includes("/")) {
    ...
    // If it had only 1 slash (e.g. "RELIANCE/OTC"), it doesn't have a quote currency in the symbol
    if (slashCount === 1 && (symbol.toUpperCase().endsWith("/OTC") || symbol.toUpperCase().endsWith("OTC"))) {
      base = clean;
      quote = "OTC";
  ...
  // Normalize quote for BideX: BideX uses USD for cryptos and commodities instead of USDT
  const cryptoCommodityBases = [
  ...
  if (cryptoCommodityBases.includes(base.toUpperCase())) {
    quote = "USD";
  }

  // If quote is empty (like for Indian stocks e.g. "SBIN"), just return "TICKER (OTC)"
  if (!quote || quote === "OTC") {
    return `${base.toUpperCase()} (OTC)`;
  }

  return `${base.toUpperCase()}/${quote.toUpperCase()} (OTC)`;
}
