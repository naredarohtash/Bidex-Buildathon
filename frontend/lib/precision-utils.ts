...
/**
 * Helper to check if a symbol is a Forex fiat pair (e.g., EUR/USD or EURUSD)
 */
export function isForexSymbol(symbol: string): boolean {
  if (!symbol) return false;
  const clean = symbol.replace(/ \((OTC)\)|\/OTC|_OTC|OTC/gi, "").trim();
  ...
  if (parts.length !== 2) {
    if (clean.length === 6) {
      ...
      const fiatCurrencies = new Set([
        "USD", "EUR", "GBP", "JPY", "CHF", "CAD", "AUD", "NZD", "ZAR", "INR",
        "CNY", "RUB", "SGD", "HKD", "TRY", "MXN", "SEK", "NOK", "DKK"
      ]);
      return fiatCurrencies.has(base) && fiatCurrencies.has(quote);
    }
    return false;
  }
  ...
  return fiatCurrencies.has(parts[0].toUpperCase()) && fiatCurrencies.has(parts[1].toUpperCase());
}
...
export function formatPriceWithPrecision(
  ...
  const precision = metadata?.precision?.price ?? (symbol && isForexSymbol(symbol) ? 5 : 2);
  return price.toFixed(precision);
}

/**
 * Format price for binary interface using localestring (with thousands separators and correct precision)
 */
export function formatBinaryPrice(
  price: number,
  symbol?: string,
  metadata?: MarketMetadata
): string {
