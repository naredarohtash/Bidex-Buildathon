...
/**
 * The one horizontal frame the header, the page body and the footer all sit in.
 *
 * They had three: the header was `container mx-auto px-4` with no max-width,
 * the footer `container mx-auto px-6 sm:px-8 max-w-7xl`, the homepage
 * `container mx-auto px-6`. So the logo in the header, the headline under it
 * and the logo again in the footer each started at a different x — visibly
 * stepped down the page on any wide screen.
 *
 * Exported as one string rather than repeated, because three copies of a
 * measurement is how they came to disagree in the first place.
 *
 * Full width, by preference: it was briefly capped at max-w-7xl, which on a
 * 1440px screen left 160px of margin either side and on 1920 left 320px — the
 * page reading as a column floating in the middle of the screen rather than as
 * the product filling it. There is no max-width now, only a gutter that grows
 * with the viewport, so the terminal's own edge-to-edge feel carries over. The
 * measures that actually need a limit set their own: the hero paragraph caps at
 * 520px, the market board at 400px.
 */
export const PAGE_CONTAINER = "w-full px-6 sm:px-10 lg:px-14";
