...
export type NotificationType =
  ...
  | "trade_win"
  | "trade_loss"
  /* A trade that ended without a winner: a DRAW, where the stake comes back
     untouched. It had no type, so it produced no notification at all — a
     position vanished from the list and the money reappeared in the balance
     with nothing said about either, which is the one outcome a trader is most
     likely to think they imagined. */
  | "trade_refund"
  | "order_placed"
  | "order_cancelled"
  | "alert_triggered"
...
export interface TradingNotification {
  ...
  data?: {
    ...
    entryTime?: number;
    expiryTime?: number;
    durationMinutes?: number; // Pre-computed planned trade duration in minutes
    currency?: string;         // Account display currency (e.g. "INR", "USDT", "USD")
    // Alert-related
    alertId?: string;
    targetPrice?: number;
    currentPrice?: number;
...
export const DEFAULT_SOUND_CONFIG: SoundConfig = {
  ...
  volume: 0.5,
  sounds: {
    // Trade sounds are owned by the audio-feedback engine (played from the
    // order panel / trading interface). They are muted here so the notification
    // toast for a trade doesn't play a SECOND overlapping sound.
    order_placed: false,
    trade_win: false,
    trade_loss: false,
    // Non-trade notifications are owned by this sound manager.
...
export const DEFAULT_NOTIFICATION_SETTINGS: NotificationSettings = {
  ...
  typePreferences: {
    ...
    trade_refund: { enabled: true, showToast: true, playSound: true, pushNotification: true },
...
export const NOTIFICATION_TYPE_INFO: Record<NotificationType, {
  ...
  trade_refund: { label: "Trade Refunded", icon: "↩", color: "zinc" },
