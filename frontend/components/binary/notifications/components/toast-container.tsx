...
export const ToastContainer = memo(function ToastContainer({
  ...
  const _activeToasts = useTradingNotificationsStore((state) => state.activeToasts);
  // Trade results are rendered inside the chart area by ChartResultToasts, in
  // the card that sits bottom-right of the chart. Excluded here so they do not
  // also appear in the generic top-right stack, which has a different shape and
  // a different position — a refund was showing in both, and the one at the top
  // was the wrong design for a trade result.
  const activeToasts = _activeToasts.filter(
    (n) => n.type !== "trade_win" && n.type !== "trade_loss" && n.type !== "trade_refund"
  );
