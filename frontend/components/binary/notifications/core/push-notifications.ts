...
class PushNotificationServiceClass {
  ...
  async showTradingNotification(
    ...
    return this.show({
      ...
      /* Silent, because the app has already made its own sound.

         A Notification with silent:false makes the OPERATING SYSTEM play its
         chime — outside our audio code, deaf to the in-app volume setting, and
         nothing like the soft sines the audio engine plays. On a settled trade
         that was the second, sharper sound: the OS tone layered over ours.
         Every notification arriving here has already had its sound decided by
         the notification service, so the OS must not add another. */
      silent: true,
    });
  }
  ...
  private getNotificationConfig(type: NotificationType): {
    ...
    const configs: Record<NotificationType, { icon: string; badge?: string }> = {
      ...
      trade_refund: { icon: "/icons/notification.png" },
