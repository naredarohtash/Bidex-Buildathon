...
const ProfileInfo = ({ square = false }: ProfileInfoProps) => {
  ...
  const handleSignOut = async () => {
    // Unconditional: logout clears the local session whether or not the server
    // call succeeds, and leaving someone signed in because a request failed is
    // the one outcome this button must never have.
    await logout();
    router.push("/");
  };
      ...
      <DropdownMenuTrigger asChild className="cursor-pointer">
        <div className={`flex items-center ${square ? "h-10 w-10 justify-center" : ""}`}>
          <Avatar className={square
            ...
            />
            <AvatarFallback className={`bg-primary text-primary-foreground font-medium text-sm flex items-center justify-center ${square ? "rounded-none" : ""}`}>
