...
export function TableContent({ viewContent, columns }: TableContentProps) {
  ...
  /* extraRowActions has to count here too. This decides whether the actions
     COLUMN exists at all — without it the cell is never rendered, so RowActions
     never mounts and its own check never runs. A page whose only action is a
     custom one (canEdit/canDelete off, no viewLink) silently lost it. */
  const hasExtraRowActions = typeof tableConfig.extraRowActions === "function";
  const showActions =
    hasEditAction || hasDeleteAction || hasViewAction || hasExtraRowActions;
