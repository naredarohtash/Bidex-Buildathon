...
export function RowActions({ row }: RowActionsProps) {
  ...
  /* 7) If there is nothing to offer, hide the entire menu.
     extraRowActions counts. It is rendered further down like any other entry,
     but was left out of this test — so a table whose only action came from
     extraRowActions (canEdit/canDelete off, no viewLink) returned null here and
     the menu button never appeared at all. The caller passes a render function,
     sees no error, and gets nothing. */
  const hasExtraActions = typeof tableConfig.extraRowActions === "function";
  const hasAnyAction = hasViewAction || hasEditAction || hasDeleteAction || hasExtraActions;
  if (!hasAnyAction) {
    return null;
  }
