...
function createBaseSchemaForType(
  ...
  switch (type) {
    ...
    case "date": {
      /* An empty optional date is not an invalid date.
      
         `formatDataForForm` fills every unset field with "", and `new Date("")`
         is NaN, so an optional date field that the record simply does not have
         failed with "Invalid date" — on every save, whether or not anybody
         touched it. There was no way to save a user with no date of birth.
      
         Emptiness is judged against `optional` here rather than left to the
         `.optional()` wrapper below, because that wrapper only admits
         `undefined` and the form never holds `undefined` for a text-like
         field. */
 ...
 *
 * `required` decides what an empty select starts as. A required one takes its
 * first option, which is what makes a new record arrive as ACTIVE rather than
 * as nothing. An optional one starts empty — it used to take the first option
 * too, so creating a user silently set their gender to Male, their time zone
 * to UTC, their country to whichever happened to sort first, and, once the
 * identity check became a control, their KYC to Verified. Nobody chose any of
 * that, and none of it was visible as a choice having been made.
 */
...
export const getDefaultValues = (
  ...
  columns.forEach((column) => {
    ...
    defaults[column.key] = getDefaultValueForType(
      column.type,
      options,
      fieldConfig?.required ?? column.required
    );
  });
