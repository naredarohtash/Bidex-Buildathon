          ...
          render={({ field, fieldState: { error } }) => {
            ...
            return (
              <FormItem className="col-span-full space-y-2">
                ...
                <FormControl>
                  {/* A portrait is not a cover image.
                  
                      This drop zone is the full width of the form and, at the
                      default size, 320px tall — a banner, for a field whose
                      whole content is one small round avatar. Asking for `xs`
                      now caps the width too, so it reads as a picture of a
                      person rather than a hero image, and drops the file
                      metadata strip that belongs to a media library. */}
                  <div className={cn(config.image.size === "xs" && "max-w-[240px]")}>
                    <ImageUpload
                      ...
                      showMetadata={config.image.size !== "xs"}
                    />
                  </div>
                </FormControl>
                {imageDescription && (
