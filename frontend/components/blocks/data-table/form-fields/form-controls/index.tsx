...
export function FormControls({
  ...
  switch (column.type) {
    ...
    case "image":
      /* `size` was dropped here, so every image field in every admin form drew
         the same 320px-tall, full-width drop zone whatever the column asked
         for. An avatar is a portrait: at `xs` it is a small tile, and capping
         the width stops it reading as a cover image. */
      return (
        <div className={cn("w-full", column.size === "xs" && "max-w-[200px]")}>
          {displayTitle && (
            <label className="mb-1 text-sm font-medium block">
              {displayTitle}
            </label>
