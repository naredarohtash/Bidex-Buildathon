...
export interface DataTableProps extends TableConfig {
  ...
  /**
   * Where the create and edit forms are drawn.
   *
   * `"page"` (the default, and what every table did before this existed)
   * replaces the whole table with the form. `"dialog"` keeps the list on
   * screen and opens the form over it — worth it for short forms, where
   * losing the row you clicked costs more than the extra room a page gives.
   * See views/form-dialog.
   */
  formMode?: "page" | "dialog";
}
...
export interface ColumnDefinition {
  ...
  /** Image fields only: how large the upload area is drawn. "xs" is a
      portrait tile; the default is a full-width drop zone. */
  size?: "xs" | "sm" | "md" | "lg" | "xl" | "default" | "adaptive";
