// /server/api/exchange/binary/leaderboard/names.ts

/**
 * Name pools for the synthetic leaderboard, one per country.
 *
 * Names are drawn from the pool belonging to the flag beside them, so a
 * Bangladeshi flag never sits next to a Brazilian name. Where a culture writes
 * the family name first — Vietnamese, and Chinese — the pool says so and the
 * generator assembles the name in that order rather than pasting a Western
 * shape onto it.
 *
 * `middle` exists for the same reason: "Nguyễn Văn Hùng" and "Phạm Thị Thanh"
 * are the ordinary shape of a Vietnamese name, and dropping the middle element
 * produces something no Vietnamese person is called.
 *
 * These are common given names and surnames, not people. Any collision with a
 * real person is the same coincidence as two strangers sharing a name.
 */

export interface NamePool {
  /* Split by gender because the board is about 70% male, which is what a retail
     binary book looks like — and because a Vietnamese middle element is
     gendered too: Văn is a man's, Thị is a woman's. One mixed list of given
     names cannot express either. */
  male: string[];
  female: string[];
  last: string[];
  /** Family name first, e.g. Vietnamese and Chinese. */
  order?: "last-first";
  middleMale?: string[];
  middleFemale?: string[];
  /** Slavic surnames take a feminine ending: Ivanov → Ivanova. */
  feminiseSurname?: boolean;
}
...
/**
 * Which name pools a country draws from, when it has more than one.
 *
 * Weighted roughly by population, so the board's Indian traders are mostly from
 * the Hindi belt and the south, the way the customer base is.
 */
export const REGIONAL_POOLS: Record<string, [string, number][]> = {
  IN: [
    ["IN_NORTH", 42],
    ["IN_SOUTH", 24],
    ["IN_BENGAL", 12],
 ...
 * Handle shapes.
 *
 * A real board is not a list of passport names. Roughly a fifth of traders on
 * any platform sign up as "Rehan Trader" or "fx_ayesha_92", and a page of
 * nothing but Firstname Lastname is the tell that a list was generated.
 */
