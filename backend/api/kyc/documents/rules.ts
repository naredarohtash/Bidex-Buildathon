// /server/api/kyc/documents/rules.ts

import { postcodeFor, type PostcodeSpec } from "./postcodes";

/**
 * What each country accepts as identity, and how many photos it takes.
 *
 * The whole verification flow reads from here: which documents a resident of a
 * country is offered, what their number should look like, and whether we ask
 * for one side or two. Adding a country is a row in this file and nothing else.
 *
 * Three things worth knowing before editing it:
 *
 *  - The first document listed for a country is the one the form leads with,
 *    because it is what businesses there actually ask for. Aadhaar before
 *    passport in India; CNIC before anything in Pakistan.
 *  - `sides: 1` is the exception and is always deliberate. PAN, Hong Kong's
 *    HKID, Mexico's CURP and Indonesia's KTP carry everything on the front —
 *    Indonesia's KTP is genuinely blank on the back. Everything else is two.
 *  - Patterns reject typos, not forgeries. A number matching the pattern proves
 *    the person typed something of the right shape; nothing here checks it
 *    exists. Where a country's format varies too much to pin down, the pattern
 *    is deliberately loose rather than wrong — a rule that rejects a real
 *    document is worse than one that accepts a malformed one, because a human
 *    reads every application anyway.
 *
 * A passport is appended to every country automatically. It is the one document
 * accepted everywhere, and repeating it 56 times invites 56 chances to differ.
 */

export interface DocumentSpec {
  id: string;
  label: string;
  /** Photos required: 2 means front and back. */
  sides: 1 | 2;
  /** Anchored regex the number must match once normalised. */
  pattern: string;
  placeholder: string;
  hint: string;
}
...
export interface CountrySpec {
  ...
  /** How that country writes a postcode, and whether it has one at all. */
  postcode?: PostcodeSpec;
}

/* Accepted everywhere, and last everywhere: it is the fallback, not the ask. */
export const PASSPORT: DocumentSpec = {
  id: "passport",
  label: "Passport",
  sides: 1,
  pattern: "^[A-Z0-9]{6,12}$",
...
const COUNTRIES_BASE: CountrySpec[] = [
  {
    code: "IN",
    name: "India",
    documents: [
...
/**
 * Every country, each with a passport option appended and its postcode rule
 * attached.
 *
 * The postcode is joined on here rather than written into each row so the two
 * lists cannot fall out of step — adding a country is still one row, and it
 * gets the sensible default until somebody writes its real format down.
 */
export const COUNTRIES: CountrySpec[] = COUNTRIES_BASE.map((c) => ({
  ...c,
  documents: [...c.documents, PASSPORT],
  postcode: postcodeFor(c.code),
}));

export const COUNTRY_BY_CODE = new Map(COUNTRIES.map((c) => [c.code, c]));

/** Numbers are compared without the spaces, dashes and case people type. */
...
export function findDocument(countryCode: string, documentId: string): DocumentLookup | null {
  const country = COUNTRY_BY_CODE.get(String(countryCode || "").toUpperCase());
...
/**
 * Whether a number is the right shape for that document.
 *
 * Dashes are stripped before matching for the formats that are written with
 * them and stored without — a Pakistani CNIC is read off the card as
 * 12345-1234567-1 and is thirteen digits underneath.
 */
export function validateNumber(doc: DocumentSpec, value: string): string | null {
  const raw = normaliseNumber(value);
  if (!raw) return "Enter the number printed on your document";

  const re = new RegExp(doc.pattern);
  if (re.test(raw)) return null;
  if (re.test(raw.replace(/-/g, ""))) return null;
